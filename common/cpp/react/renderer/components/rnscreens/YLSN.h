...
class YLSN : public LayoutableShadowNode {
 public:
  using Shared = std::shared_ptr<const YLSN>;
...
#pragma mark - Constructors
  ...
  YLSN(const ShadowNode &sourceShadowNode, const ShadowNodeFragment &fragment);
...
#pragma mark - LayoutableShadowNode
 ...
 private:
   ...
   * returns false.
   */
  bool doesOwn(const YLSN &child) const;
   ...
   * adjustment).
   */
  void appendYogaChild(const YLSN::Shared &childNode);
   ...
   */
  YLSN &cloneChildInPlace(size_t layoutableChildIndex);
  ...
  static float yogaNodeBaselineCallbackConnector(
      ...
      float width,
      float height);
  static YLSN &shadowNodeFromContext(YGNodeConstRef yogaNode);
...
#pragma mark - Private member variables
  /*
   * List of children which derive from YLSN
   */
