...
async function generateCodegenJavaOldArch() {
  ...
  if (oldArchFiles.length === 0) {
    console.warn(
      "[RNScreens] Paper destination with codegen interfaces is empty. This might be okay if you don't have any interfaces/delegates used on Android, otherwise please check if OLD_ARCH_DIR and SOURCE_FOLDERS are set properly."
    );
  }
  ...
  oldArchFiles.forEach(oldArchFile => {
    if (!fs.existsSync(`${CODEGEN_FILES_DIR}/${oldArchFile}`)) {
      console.warn(
        `[RNScreens] ${existingFile.name} file does not exist in codegen artifacts source destination. Please check if you still need this interface/delagete.`
      );
    }
  });
...
function compareFileAtTwoPaths(filename, firstPath, secondPath) {
  ...
  if (fileA !== fileB) {
    throw new Error(
      `[RNScreens] File ${filename} is different at ${firstPath} and ${secondPath}. Make sure you commited codegen autogenerated files.`
    );
  }
}
