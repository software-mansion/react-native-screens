...
function resolveAttachedAndroidDeviceSerial() {
  ...
  const connectedNonEmulators = getDeviceIds()
  ...
  if (connectedNonEmulators.length === 0) {
    ...
    `Connected devices: ${connectedNonEmulators.join(', ')}\nUnplug ${connectedNonEmulators.length - 1} device(s) or use ${envVarKeys.adbSerial} environment variable to select a specific one.`
  );
}

function detectAndroidEmulatorName() {
  const isEmulatorConfig = process.argv.some(runtimeArg => runtimeArg.includes('android.emu'));
