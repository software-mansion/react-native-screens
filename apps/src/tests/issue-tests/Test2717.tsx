...
function NestedSecond({ navigation }: NestedStackNavigationProp) {
  return (
    <View style={[{ backgroundColor: 'lightsalmon' }, styles.container]}>
      ...
      <Button
        title="Go back to NestedHome"
        onPress={() => navigation.popTo('NestedHome')}
      />
      <Button
        title="REPLACE with NestedThird"
        onPress={() => navigation.replace('NestedThird')}
      />
    </View>
  );
}
