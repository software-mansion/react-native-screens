...
function StackHome({ navigation }: StackRouteNavProps) {
  return (
    <View style={{ flex: 1, backgroundColor: 'lightgreen' }}>
      <Button
        title="Open sheet"
        onPress={() => navigation.navigate('StackSheet')}
      />
      <Button
        title="Open StackSecond"
        onPress={() => navigation.navigate('StackSecond')}
      />
      <Button
        title="Open NestedStackHost"
        onPress={() => navigation.navigate('NestedStackHost')}
      />
    </View>
  );
}
...
function StackSecond({ navigation }: StackRouteNavProps) {
  return (
    <View style={{ flex: 1, backgroundColor: 'yellow' }}>
      <Button
        title="PopTo StackSheet"
        onPress={() => navigation.popTo('StackSheet')}
      />
      <Button
        title="PopTo StackHome"
        onPress={() => navigation.popTo('StackHome')}
      />
    </View>
  );
}
...
function NestedHome({ navigation }: NestedStackRouteNavProps) {
  return (
    <View style={{ flex: 1, backgroundColor: 'crimson' }}>
      <Button
        title="Open NestedSheet"
        onPress={() => navigation.navigate('NestedSheet')}
      />
      <Button
        title="Open StackSheet"
        onPress={() => navigation.navigate('StackSheet')}
      />
    </View>
  );
  ...
  return (
    <NavigationContainer>
      <Stack.Navigator>
        ...
        />
        <Stack.Screen
          name="NestedStackHost"
          component={NestedStackHost}
          options={{
            headerShown: false,
          }}
        />
