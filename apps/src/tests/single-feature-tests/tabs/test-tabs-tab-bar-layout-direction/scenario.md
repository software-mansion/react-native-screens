...
- iOS: The system RTL direction is set by configuring I18NIsRTL,
AppleTextDirection and NSForceRightToLeftWritingDirection to YES during the app
launch sequence.
...
- For iPhone this scenario has to be executed with a system-wide RTL
  language enabled at the device level.
- For Android each of the below steps must be executed twice: once with a system-wide RTL
...
5. Cycle through inherit → rtl → ltr → rtl → inherit.

- [ ] Expected: Tab bar direction updates immediately with each change; no
  crashes or layout freezes occur. For inherit ltr direction is applied.

---

### System/RN rtl

6. Set system/RN to RTL (I18nManager.isRTL == true), set TabsHost
   direction = inherit.
...
9. Cycle through inherit → rtl → ltr → rtl → inherit.

- [ ] Expected: Tab bar direction updates immediately with each change; no
  crashes or layout freezes occur. For inherit rtl direction is applied.
