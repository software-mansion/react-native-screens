...
1. Ensure system/RN is LTR (I18nManager.isRTL == false), set TabsHost direction = inherit
    
    Expected: Tab bar displays in LTR order (Config on left, Tab2 to its right)
2. Set system/RN to RTL (I18nManager.isRTL == true), keep TabsHost direction = inherit
    
    Expected: Tab bar displays in RTL order — (Config on right, Tab2 to its left)

**TabsHost ltr**

1. Set system/RN to RTL, set TabsHost direction = ltr
    
    Expected: Tab bar displays in LTR order — TabsHost overrides RTL from RN/system
2. Set system/RN to LTR, keep TabsHost direction = ltr
    
    Expected: Tab bar stays LTR
3. Cycle through inherit → rtl → ltr → rtl → inherit
    
    Expected: Tab bar direction updates immediately with each change, no crash or layout freeze

**TabsHost rtl**

1. Set system/RN to LTR, set TabsHost direction = rtl
    
    Expected: Tab bar displays in RTL order — TabsHost overrides LTR from RN/system
2. Set system/RN to RTL, keep TabsHost direction = rtl
    
    Expected: Tab bar stays RTL
3. Cycle through inherit → ltr → rtl → ltr → inherit
    
    Expected: Tab bar direction updates immediately with each change, no crash or layout freeze

**Precedence chain verification**
1.  System = RTL, forceRTL=false, allowRTL=false, TabsHost = inherit
    
    Expected: Tab bar is LTR (allowRTL=false blocks system RTL)
2.  System = LTR, forceRTL=true (restart), TabsHost = inherit
    
    Expected: Tab bar is RTL (forceRTL overrides system)
3.  System = LTR, forceRTL=true (restart), TabsHost = ltr
