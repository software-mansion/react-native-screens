  ...
  return (
    <ScrollView style={{ padding: 40 }}>
      <SettingsPicker
        ...
        value={stackConfig.find(c => c.name === 'Screen1')?.options?.orientation ?? 'undefined'}
        onValueChange={value => stackDispatch({ type: 'screen', name: 'Screen1', config: { orientation: value === 'undefined' ? undefined : value } })}
      />
      <SettingsPicker
        label='Tab Screen orientation'
        items={['portrait', 'landscape', 'undefined']}
        value={tabsConfig.tabConfigs.find(c => c.tabScreenProps.tabKey === 'Tab1')?.tabScreenProps.orientation ?? 'undefined'}
        onValueChange={value => tabsDispatch({ type: 'tabScreen', tabKey: 'Tab1', config: { tabScreenProps: { orientation: value === 'undefined' ? undefined : value } } })}
      />
    </ScrollView>
  );
};
...
export default function TabsAndStack() {
  return (
    <StackConfigProvider screens={{ Screen1: TabsScreen }}>
      <StackAutoconfig />
    </StackConfigProvider>
  )
}
