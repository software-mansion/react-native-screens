...
namespace winrt::RNScreens::implementation {
    ...
    void ScreenContainerViewManager::AddView(FrameworkElement parent, UIElement child, int64_t index) {
    ...
    void ScreenContainerViewManager::RemoveChildAt(FrameworkElement parent, int64_t index) {
        auto screenContainer = parent.as<ScreenContainer>();
        if (!screenContainer)
            return;
    ...
    void ScreenContainerViewManager::ReplaceChild(FrameworkElement parent, UIElement oldChild, UIElement newChild) {
        auto screenContainer = parent.as<ScreenContainer>();
        if (!screenContainer)
            return;
    ...
    void ScreenContainerViewManager::ReactContext(IReactContext reactContext) noexcept {
        m_reactContext = reactContext;
    }

    // IViewManagerWithNativeProperties
    IMapView<hstring, ViewManagerPropertyType> ScreenContainerViewManager::NativeProps() noexcept {
    ...
    ConstantProviderDelegate ScreenContainerViewManager::ExportedCustomBubblingEventTypeConstants() noexcept {
        return nullptr;
    }

    ConstantProviderDelegate ScreenContainerViewManager::ExportedCustomDirectEventTypeConstants() noexcept {
        return nullptr;
    }
