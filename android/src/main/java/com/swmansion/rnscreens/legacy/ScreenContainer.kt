...
open class ScreenContainer(
    ...
    open val topScreen: Screen?
        get() = screenWrappers.firstOrNull { getActivityState(it) === Screen.ActivityState.ON_TOP }?.screen
    ...
    open fun onUpdate() {
        createTransaction().let {
            ...
            for (fragmentWrapper in screenWrappers) {
                if (getActivityState(fragmentWrapper) === Screen.ActivityState.INACTIVE &&
            ...
            for (fragmentWrapper in screenWrappers) {
                ...
                if (activityState == Screen.ActivityState.INACTIVE) {
                    continue
                }
