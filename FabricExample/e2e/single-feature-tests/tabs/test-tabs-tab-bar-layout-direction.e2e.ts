...
async function getElementAttributes(testLabel: string): Promise<ElementAttributes> {
...
//     it('Tab Bar Layout Direction screen should be displayed with default options.', async () => {
...
//         await expect(element(by.id('react-force-rtl-picker'))).toHaveLabel('forceRTL: false');
//         await expect(element(by.id('react-allow-rtl-picker'))).toHaveLabel('allowRTL: true');
//         await expect(element(by.id('tab-bar-layout-direction-picker'))).toHaveLabel('direction: inherit');
//         await expect(element(by.id('is-rtl-information'))).toHaveText('I18nManager.isRTL == false');
//     });
...
//         await (element(by.id('tab-bar-layout-direction-picker'))).tap();
//         await expect(element(by.id('tab-bar-layout-direction-picker'))).toHaveLabel('direction: rtl');
...
//         await (element(by.id('tab-bar-layout-direction-picker'))).tap();
//         await expect(element(by.id('tab-bar-layout-direction-picker'))).toHaveLabel('direction: ltr');
...
describe('Tab Bar Layout Direction - system settings: RTL', () => {
    beforeAll(async () => {
        if (device.getPlatform() === 'ios') {
            ...
            execSync(`adb shell settings put global development_settings_enabled 1`);
            execSync(`adb shell settings put global debug.force_rtl 1`);
    ...
    afterAll(async () => {
        if (device.getPlatform() === 'ios') {
            ...
            const { execSync } = require('child_process');
            execSync(`adb shell settings put global debug.force_rtl 0`);
            await device.launchApp({ newInstance: true });
        }
    });
    ...
    it('Tab Bar Layout Direction screen should be displayed with default options.', async () => {
        ...
        await expect(element(by.id('react-allow-rtl-picker'))).toHaveLabel('allowRTL: true');
        await expect(element(by.id('tab-bar-layout-direction-picker'))).toHaveLabel('direction: inherit');
        await expect(element(by.id('is-rtl-information'))).toHaveText('I18nManager.isRTL == true');
    });
