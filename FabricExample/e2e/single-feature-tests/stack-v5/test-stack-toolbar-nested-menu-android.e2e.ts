...
// Typed wrappers over the shared matchers: every text this screen addresses is
// one of `ALL_MENU_TEXTS`, so a typo fails to compile rather than at runtime.
const menuText = (text: MenuText): Detox.NativeMatcher => textInMenu(text);
...
/** Pickers and switches sit on the scrollable settings screen behind the menu. */
...
async function withMenusClosedAfter(assertions: () => Promise<void>) {
  await closingMenuAfter(SCROLLVIEW_ID, assertions);
}
...
async function tapMenuItem(path: readonly MenuText[], item: MenuText) {
  ...
  await tapLeafItem(menuText(item), SCROLLVIEW_ID);
}
