...
export async function selectSingleFeatureTest(scenarioGroupName: string, screenName: string ) {
  ...
  await expect(element(by.id(`single-feature-tests-${scenarioGroupName}`))).toBeVisible();
  await element(by.id(`single-feature-tests-${scenarioGroupName}`)).tap();

  await expect(element(by.label(`${screenName}, i, a`))).toBeVisible();
  await element(by.label(`${screenName}, i, a`)).tap();
}
